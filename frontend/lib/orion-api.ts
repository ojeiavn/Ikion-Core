...
export type OrionUserRole = "student" | "lecturer" | "admin"
...
export interface OrionSessionRecord {
  id: string
  user_id: string
...
export interface OrionUserSummary extends OrionUser {
  active_session_count: number
}

export interface OrionAuthSession {
  user: OrionUser
  session: OrionSessionRecord
}

export interface OrionSetupStatus {
  setup_required: boolean
  user_count: number
  allowed_roles: OrionUserRole[]
  allow_dev_auth_reset: boolean
}

export interface OrionWorkspace {
  id: string
  slug: string
  name: string
...
export interface OrionWorkspaceSummary extends OrionWorkspace {
  membership_role: OrionUserRole
}

export interface OrionWorkspaceMembership {
  id: string
  workspace_id: string
  user_id: string
  role: OrionUserRole
...
export interface OrionGuidancePack {
  id: string
  workspace_id: string
  name: string
  instructions: string
...
export interface OrionExamQuestion {
  id: string
  workspace_id: string
...
export interface OrionExamAttempt {
  id: string
  workspace_id: string
  user_id: string
...
export interface OrionExamRecommendation {
  id: string | null
  question_text: string
  question_number: string | null
  topic_label: string | null
...
export interface OrionExamEvaluation {
  workspace_id: string
  exam_question_id: string | null
  question_text: string
  score: number
...
export interface OrionExamPrepProfile {
  workspace_id: string
  ...
  recommended_question: OrionExamRecommendation | null
  recent_attempts: OrionExamAttempt[]
}

export interface OrionCorpusRecord {
  id: string
  workspace_id: string
  status: string
  chunk_count: number
...
export interface OrionCorpusInspection {
  database_record: OrionCorpusRecord | null
  workspace_manifest: Record<string, unknown>
  version_manifest: Record<string, unknown> | null
}

export interface OrionCitation {
  index: number
  chunk_id: string
  asset_id: string
  asset_title: string
...
export interface OrionPlayback {
...
export interface OrionQueryEvent {
  id: string
  workspace_id: string
  user_id: string | null
...
export interface OrionConversation {
  id: string
  workspace_id: string
  user_id: string | null
  title: string
...
export interface OrionConversationMessage {
  id: string
...
export interface OrionAskResponse {
  workspace_id: string
  query: string
  conversation_id: string | null
  ...
  guidance_applied: string[]
  playback: OrionPlayback | null
  playback_segments: OrionPlayback[]
}

export interface OrionInsights {
  workspace_id: string
  generated_at: string
  query_count: number
  ...
  exam_prep?: OrionExamPrepProfile | null
}

export interface OrionTopicNotesResponse {
  workspace_id: string
  generated_at: string
  topics: Array<{
    id: string
...
type OrionFetchInit = RequestInit & {
  timeoutMs?: number
}

export async function orionFetch<T>(path: string, init?: OrionFetchInit): Promise<T> {
  ...
  try {
    ...
    if (error instanceof DOMException && error.name === "AbortError") {
      throw new Error("Request timed out. The Orion backend did not respond in time.")
    }
