...
@lru_cache(maxsize=1)
def get_runtime() -> IkionCoreRuntime:
    return create_runtime()


def create_app(runtime: IkionCoreRuntime | None = None) -> FastAPI:
    active_runtime = runtime or get_runtime()
    app = FastAPI(title="Ikion Core Backend", version="0.1.0")
    app.add_middleware(
        CORSMiddleware,
    ...
    def stream_asset_content(workspace_id: str, asset_id: str, raw_request: Request) -> Response:
        ...
        if drive_file_id:
            ...
            # This keeps playback fully external-source backed in Ikion.
        ...
        # The expected behavior is authenticated in-app streaming via Ikion.
