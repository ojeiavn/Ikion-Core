...
class TranscriptGenerationService:
    def __init__(
        ...
        self._executor = ThreadPoolExecutor(max_workers=1, thread_name_prefix="ikion-transcripts")
        self._lock = Lock()
        ...
        try:
            ...
            transcript_asset = self.ingestion.register_transcript_segments(
                ...
                metadata={
                    ...
                    "transcript_origin": "ikion_auto_generated",
    ...
    def _transcribe_with_openai(self, media_path: Path, video_asset: Asset) -> list[TranscriptSegment]:
        if not self.config.openai_api_key:
            raise ValueError("Automatic transcript generation requires IKION_OPENAI_API_KEY or OPENAI_API_KEY.")
        prompt = (
