...
class ExamPracticeService:
    ...
    def evaluate_answer(
        ...
        payload = {
            ...
            "guidance": guidance,
            "evaluation_text": clean_text(str(evaluation.get("evaluation", "") or "")) or "Use stronger, more explicit evidence from Orion's cited materials.",
    ...
    def build_personalized_guidance(self, workspace_id: str, user_id: str | None, query: str) -> str | None:
        ...
        guidance_bits = [
            ...
            f"Ask Orion exam-chat readiness component: {prompt_context.get('chat_readiness_score', 0)}/100.",
        ]
        if focus_topics:
    ...
    def _why_this_now(
        ...
        reasons: list[str] = []
        if focus_topics:
            reasons.append(f"It targets the topics Orion currently sees as weakest: {', '.join(focus_topics[:3])}.")
        if attempts:
        ...
        for row in query_rows[:8]:
            ...
            if metrics.get("exam_related") and float(metrics.get("chat_readiness_score", 0.0) or 0.0) < 0.45:
                weak_exam_chats += 1
        if weak_exam_chats:
            reasons.append("Your recent Ask Orion exam interactions show the same topic still needs reinforcement.")
        if mark_scheme_available:
